<p align="center">
        ...
        <p align="center">
          <em>快速取得協助 → <a href="https://discord.gg/VxfP4Vx3gX">加入 Discord</a> • 比較喜歡非同步？→ <a href="https://www.reddit.com/r/MaveCode/">加入 r/MaveCode</a></em>
        </p>
...
> 之後，MaveCode 將繼續開發這個專案。感謝 Roo 團隊所建立的一切。
>
> 核心團隊由先前曾為 Roo 做出貢獻、並且非常在乎這個外掛的開發者所組成。
> 我們會持續更新模型、修正 bug，並推出新功能，也計劃仔細傾聽讓這個外掛
> 如此特別的社群。歡迎加入我們，一起在
> [Discord](https://discord.gg/VxfP4Vx3gX)、
> [Reddit](https://www.reddit.com/r/MaveCode)，或是
> [建立 PR 或 issue](https://github.com/MaveCode-Org/MaveCode)。
>
> _-MaveCode Team_

## 從 Roo Code 遷移到 MaveCode

你可以在 [Roo→Zoo 遷移指南](https://docs.mavecode.dev/roo-to-zoo-migration) 中找到從 Roo Code 遷移到 MaveCode 的快速說明。我們希望在大家轉移過程中盡可能提供協助，這也是我們設立 [Reddit](https://www.reddit.com/r/MaveCode) 和 [Discord](https://discord.gg/VxfP4Vx3gX) 社群的原因。如果你遇到問題或有任何疑問，歡迎加入後直接提問。

## MaveCode 在 Roo Code 之後新增的功能

MaveCode 以 Roo Code 建立的基礎持續擴充，新增了：
...
- **更多連線方式** — 新增並擴充 MaveCode、Moonshot、Kimi Code、Kenari、Friendli、OpenCode Go 等眾多供應商。
...
更多資訊：[使用模式](https://docs.mavecode.dev/basic-usage/using-modes) • [自訂模式](https://docs.mavecode.dev/advanced-usage/custom-modes)

## 資源

- **[文件](https://docs.mavecode.dev):** 安裝、設定和掌握 MaveCode 的官方指南。
- **[Discord 伺服器](https://discord.gg/VxfP4Vx3gX):** 加入社群以獲得即時協助和討論。
- **[Reddit 社群](https://www.reddit.com/r/MaveCode):** 分享您的經驗，看看其他人正在建立什麼。
- **[GitHub Issues](https://github.com/MaveCode-Org/MaveCode/issues):** 回報問題並追蹤開發進度。
- **[功能請求](https://github.com/MaveCode-Org/MaveCode/discussions/categories/feature-requests?discussions_q=is%3Aopen+category%3A%22Feature+Requests%22+sort%3Atop):** 有想法嗎？與開發人員分享。
...
有幾種方法可以執行 MaveCode 擴充功能：

### 開發模式（F5）

若要進行開發，請使用 VSCode 的內建偵錯功能：

在 VSCode 中按 `F5`（或前往 **執行** → **開始偵錯**）。這將在執行 MaveCode 擴充功能的新 VSCode 視窗中開啟。
...
1.  首先，建置 VSIX 套件：
    ...
    pnpm vsix
    ```
2.  將在 `bin/` 目錄中產生一個 `.vsix` 檔案（例如 `bin/mave-code-<version>.vsix`）。
...
**請注意**，MaveCode **不**對與 MaveCode 相關的任何程式碼、模型或其他工具、任何相關的第三方工具或任何由此產生的輸出作出任何陳述或保證。您承擔使用任何此類工具或輸出的**所有風險**；此類工具均按**「原樣」**和**「可用」**的基礎提供。此類風險可能包括但不限於智慧財產權侵權、網路漏洞或攻擊、偏見、不準確、錯誤、缺陷、病毒、停機、財產損失或損害和/或人身傷害。您對自己使用任何此類工具或輸出負全部責任（包括但不限於其合法性、適當性和結果）。
...
**盡情享受 MaveCode！** 不論你是讓它保持短牽繩控制，還是讓它自主行動，我們都迫不及待想看看你會打造出什麼。如果你有問題或功能想法，請開一個 [issue](https://github.com/MaveCode-Org/MaveCode/issues) 或發起一個 [discussion](https://github.com/MaveCode-Org/MaveCode/discussions)。祝你寫程式愉快！
