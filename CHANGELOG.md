...
- Dismiss the welcome screen after successful MaveCode sign-in (#961 by @JohnCanty, PR #962 by @JamesRobert20)
...
- Fix(mave-gateway): enable image attach for MaveCode and Vercel AI Gateway models based on live vision-capability tags instead of a static allowlist (PR #897 by @JamesRobert20)
...
- Add LiteLLM support for forwarding taskId as X-Mave-Session-ID request header for better request tracing (#590 by @awschmeder, PR #591 by @awschmeder)
...
- Add MaveCode provider with auth callback and multi-profile token sync (PR #344 by @JamesRobert20, PR #345 by @JamesRobert20, PR #347 by @JamesRobert20)
...
- Show MaveCode identity in outbound provider activity logs (#203 by @yfdyh000, PR #219 by @app/roomote)
...
- Rename MaveCode in VS Code code actions (#328 by @rrewll, PR #329 by @rrewll)
...
- Fix API requests hanging indefinitely on VS Code 1.122.0+ when MaveCode could not find the bundled ripgrep binary after the `@vscode/ripgrep-universal` rename (#381 by @greatgradz-svg, PR #248 by @0xMink).

All notable changes to MaveCode will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and MaveCode uses [Semantic Versioning](https://semver.org/spec/v2.0.0.html).
...
- Merge the Roo Code upstream sunset into MaveCode and pull in related handoff updates (PR #123 by @edelauna)
...
- Fix Discord invite links that still pointed to the old MaveCode server (PR #107 by @app/roomote)
- Fix support links that opened the wrong GitHub repository (PR #77 by @app/roomote)
- Refresh MaveCode branding across docs and metadata (PR #85 by @taltas)
- Clarify MaveCode migration messaging in the README (PR #99 by @taltas)
...
- Historical pre-MaveCode marketplace publication metadata was removed from the active fork.
...
The entries below are preserved from the upstream Roo Code project history before the MaveCode marketplace handoff.
...
- Add blog section with initial posts to the legacy upstream website (PR #11127 by @app/roomote)
...
- Web: Website improvements to https://mavecode.invalid/ (thanks @brunobergher!)
...
- New Roo Code community Discord at https://mavecode.invalid/discord!
