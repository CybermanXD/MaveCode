...
async function createSymlinkOrSkip(
	...
	try {
		...
		// Windows requires Developer Mode or SeCreateSymbolicLinkPrivilege. The
		// product behavior is covered wherever the host permits creating links.
		skip()
		return false
	}
}
...
describe("rules service", () => {
	...
	it("skips symlinked directory rules outside the rules directory", async ({ skip }) => {
		...
		if (!(await createSymlinkOrSkip(targetRulesDir, path.join(projectRulesDir, "linked"), "dir", skip))) return
	...
	it("round-trips symlinked directory rules that stay inside the rules directory", async ({ skip }) => {
		const projectRulesDir = path.join(cwd, ".roo", "rules")
	...
	it("skips symlinked rule file targets outside the rules directory", async ({ skip }) => {
		const projectRulesDir = path.join(cwd, ".roo", "rules")
		const targetRulePath = path.join(tempDir, "linked-rule.md")
		await fs.mkdir(projectRulesDir, { recursive: true })
		await fs.writeFile(targetRulePath, "# Linked")
		if (!(await createSymlinkOrSkip(targetRulePath, path.join(projectRulesDir, "linked-file.md"), "file", skip)))
			return
	...
	it("discovers a symlinked rule file target inside the rules directory", async ({ skip }) => {
		const projectRulesDir = path.join(cwd, ".roo", "rules")
	...
	it("skips broken symlinks while scanning rules", async ({ skip }) => {
		const projectRulesDir = path.join(cwd, ".roo", "rules")
		await fs.mkdir(projectRulesDir, { recursive: true })
		if (
			!(await createSymlinkOrSkip(
				path.join(tempDir, "missing-target"),
				path.join(projectRulesDir, "broken.md"),
				"file",
				skip,
			))
		)
