...
export async function getRipgrepDiagnostic(vscodeAppRoot: string): Promise<string> {
	...
	const lines: string[] = [
		`MaveCode Ripgrep Diagnostic (${new Date().toISOString()})`,
...
/**
 * Registers the `mave-code.showRipgrepDiagnostic` command. Thin wrapper —
...
export function registerRipgrepDiagnosticCommand(): vscode.Disposable {
	const channel = vscode.window.createOutputChannel("MaveCode Ripgrep Diagnostic")
	const command = vscode.commands.registerCommand(getCommand("showRipgrepDiagnostic"), async () => {
		...
		await vscode.window.showInformationMessage("MaveCode: ripgrep diagnostic copied to clipboard.")
	})
