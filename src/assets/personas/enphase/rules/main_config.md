...
Source reviewed: `Main References/Enphase_Main_Reference.html`
...
- Future Enphase email builds should reuse classes from `Enphase_Main_Reference.html` first. Do not create new classes unless none of these solve the requirement.
...
- Use existing font helper classes for mobile scaling before inventing any new responsive font class.
- Use image/icon sizing classes from the inventory before creating custom image classes.
...
- Do not create new CSS if a matching utility/helper already exists in any of the three approved reference sources.
- If none of the approved sources contain the needed class/behavior, document the gap in this config before introducing a new helper in a project.
...
- For this body-copy pattern, pair desktop inline `font-size:18px; line-height:28px;` with the mobile helper `.em_font16`.
...
- `Block 27`: two black CTAs in one row; uses `.em_cta`; includes known `stylke` typo to fix before reuse.
...
- Black body module:
  ...
  - Use white body copy with `18px/28px`, `.em_font16`, and inline styles on each text cell.
...
- Do not forget change logs for generated files in project folders.
...
- Create a concise change log in the project folder.
- Add any new learning back into this config file.

## Roo Self-Training Rules for Enphase Builds
- Treat this config as the operating memory for all future Enphase email work.
...
- Avoid creating new CSS classes unless the project requires behavior not already covered.
- If creating a new CSS helper, name it in the `em_` style family and document it in this config after use.
...
- Every generated project HTML file should have a matching concise change log.
- Change logs should be newest-first.
- Each entry should state:
  - Date/time or version if available.
  - What changed.
  - Why it changed.
  - Files affected.
- Do not write long narrative change logs; keep them concise and production-focused.
- If a bug fix becomes reusable, add it to this config.
...
- Before finalizing any project, mentally run this sequence:
  ...
  - Is the change log created?
- If any answer is uncertain, inspect the reference/config again before writing final output.

## Known Issues Found in Reference
- `.em_aside6` has invalid CSS padding: `padding: 40 15px 10px 15px !important;` should be `padding: 40px 15px 10px 15px !important;`.
- `stylke="width:30px; font-size:0px;"` typo appears in a CTA spacer and should be `style`.
- Some anchor `target` values use `target="blank"`; use `target="_blank"`.
...
- One link contains two campaign query fragments: `...?utm_campaign=evergreen?utm_campaign=%%=v(@utm_campaign)=%%`; correct to a single query parameter pattern before sending.
- Block 24 has a table with a direct `td` child before an explicit `tr`; wrap all `td` elements inside `tr` for valid email markup.
...
## Project Usage Rule
- For each new project, use `Enphase_Main_Reference.html` as a block library.
...
- Add a concise change log for every generated HTML file.
- If a new issue or reusable fix is discovered, append the learning to this `main_config.md`.

## Project Learning: HTML Review
- HTML review learnings have been promoted into the main config sections above so they apply to future Enphase builds.
- Key reusable rules added across the config: corrected/fixed HTML files override assumptions; black body modules use white `18px/28px` live text; normal body spacing should be padding-based; bullet rows should follow the fixed nested-table pattern; and black-background body links should use orange `#E96F1A` with underline.
