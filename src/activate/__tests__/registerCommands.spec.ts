...
describe("registerCommands handlers", () => {
	...
	it("settingsButtonClicked posts both settingsButtonClicked and didBecomeVisible actions", () => {
		handlers["mave-code.settingsButtonClicked"]()
	...
	it("historyButtonClicked posts historyButtonClicked action", () => {
		handlers["mave-code.historyButtonClicked"]()
	...
	it("marketplaceButtonClicked posts marketplaceButtonClicked action", () => {
		handlers["mave-code.marketplaceButtonClicked"]()
	...
	it("acceptInput posts acceptInput message", () => {
		handlers["mave-code.acceptInput"]()
	...
	it("toggleAutoApprove awaits postMessage with toggleAutoApprove action", async () => {
		...
		const handlerPromise = handlers["mave-code.toggleAutoApprove"]() as Promise<unknown>
		let settled = false
	...
	it("focusInput awaits postMessage on the registered provider when a sidebar panel is active", async () => {
		...
		const handlerPromise = handlers["mave-code.focusInput"]() as Promise<unknown>
		let settled = false
		void handlerPromise.then(() => {
			settled = true
		})
	...
	it("focusInput does not post when no sidebar panel is active", async () => {
		await handlers["mave-code.focusInput"]()
	...
	it.each([
		{ command: "mave-code.settingsButtonClicked", prefix: "settingsButtonClicked", expectedCalls: 2 },
		{ command: "mave-code.historyButtonClicked", prefix: "historyButtonClicked", expectedCalls: 1 },
		{ command: "mave-code.marketplaceButtonClicked", prefix: "marketplaceButtonClicked", expectedCalls: 1 },
		{ command: "mave-code.acceptInput", prefix: "acceptInput", expectedCalls: 1 },
	])(
	...
	it("toggleAutoApprove logs to outputChannel when postMessageToWebview rejects", async () => {
		...
		await handlers["mave-code.toggleAutoApprove"]()
	...
	it("plusButtonClicked calls evictCurrentTask on the visible provider", async () => {
		...
		await handlers["mave-code.plusButtonClicked"]()
