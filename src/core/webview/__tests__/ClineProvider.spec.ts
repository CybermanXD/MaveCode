...
vi.mock("../../../services/mave-code-auth", () => ({
	...
	disconnectMaveCode: vi.fn(),
}))

beforeEach(async () => {
	const { getCachedMaveCodeToken } = await import("../../../services/mave-code-auth")
	vi.mocked(getCachedMaveCodeToken).mockReturnValue("mave_ext_test_session")
})
...
vi.mock("../../../services/cloud/CloudCompatibility", () => ({
	CloudService: {
	...
	getRooCodeApiUrl: vi.fn().mockReturnValue("https://cloud.mavecode.invalid"),
}))
...
describe("ClineProvider", () => {
	...
	test("resolveWebviewView sets up webview correctly", async () => {
		...
		expect(mockWebviewView.webview.html).toContain("<title>MaveCode</title>")
	})

	test("renders a secure, accessible manual sign-in gate without initializing the full UI", async () => {
		const { getCachedMaveCodeToken } = await import("../../../services/mave-code-auth")
		vi.mocked(getCachedMaveCodeToken).mockReturnValue("")
		const getStateSpy = vi.spyOn(provider, "getState")
		const listenerDisposable = { dispose: vi.fn() }
		mockWebviewView.webview.onDidReceiveMessage.mockReturnValue(listenerDisposable)

		await provider.resolveWebviewView(mockWebviewView)

		expect(mockWebviewView.webview.options).toEqual({ enableScripts: true })
		expect(mockWebviewView.webview.html).toContain("Sign in with Google")
		expect(mockWebviewView.webview.html).toContain('id="mavecode-sign-in"')
		expect(mockWebviewView.webview.html).toContain('role="status"')
		expect(mockWebviewView.webview.html).toMatch(/Content-Security-Policy[^>]+script-src 'nonce-[A-Za-z0-9]+'/)
		expect(mockWebviewView.webview.html).toContain('vscode.postMessage({ type: "startMaveCodeSignIn" })')
		expect(getStateSpy).not.toHaveBeenCalled()
		expect(mockWebviewView.onDidChangeVisibility).not.toHaveBeenCalled()
		expect(mockWebviewView.webview.html).not.toContain('<div id="root"></div>')

		await provider.dispose()
		expect(listenerDisposable.dispose).toHaveBeenCalledOnce()
	})

	test("handles manual sign-in messages through the coalesced transaction flow and permits failed-launch retries", async () => {
		const { getCachedMaveCodeToken, startMaveCodeSignIn } = await import("../../../services/mave-code-auth")
		vi.mocked(getCachedMaveCodeToken).mockReturnValue("")
		vi.mocked(startMaveCodeSignIn).mockResolvedValueOnce(false).mockResolvedValueOnce(true)
		mockWebviewView.webview.onDidReceiveMessage.mockReturnValue({ dispose: vi.fn() })

		await provider.resolveWebviewView(mockWebviewView)
		const messageHandler = mockWebviewView.webview.onDidReceiveMessage.mock.calls[0][0]
		await messageHandler({ type: "unrelated" })
		await messageHandler({ type: "startMaveCodeSignIn" })
		await messageHandler({ type: "startMaveCodeSignIn" })

		expect(startMaveCodeSignIn).toHaveBeenCalledTimes(2)
		expect(mockPostMessage).toHaveBeenNthCalledWith(1, { type: "maveCodeSignInLaunchComplete", launched: false })
		expect(mockPostMessage).toHaveBeenNthCalledWith(2, { type: "maveCodeSignInLaunchComplete", launched: true })
	})
...
describe("webviewMessageHandler no-floating-promises coverage", () => {
	...
	it("covers changed cloud sign-out and rate-limit error responses", async () => {
		const { CloudService } = await import("../../../services/cloud/CloudCompatibility")
...
describe("ClineProvider - Router Models", () => {
	...
	test("handles requestRouterModels with individual provider failures", async () => {
		...
		vi.mocked(getModels)
			...
			.mockResolvedValueOnce(mockModels) // mave-gateway success
...
describe("ClineProvider - Comprehensive Edit/Delete Edge Cases", () => {
	...
	describe("MaveCode auth profile sync", () => {
		describe("handleMaveCodeCallback", () => {
			it("creates a MaveCode profile when none exists", async () => {
				vi.spyOn(provider, "getState").mockResolvedValue({
					apiConfiguration: { maveGatewayModelId: "anthropic/claude-sonnet-4" },
				} as any)
				...
				await provider.handleMaveCodeCallback("mave_ext_token")

				expect(postMessageSpy).toHaveBeenCalledWith({ type: "maveGatewayCredentialsReady" })
			...
			it("updates every mave-gateway profile and activates only the active one", async () => {
				vi.spyOn(provider, "getState").mockResolvedValue({
					apiConfiguration: { maveGatewayModelId: "anthropic/claude-sonnet-4" },
				} as any)
				vi.spyOn(provider.contextProxy, "getProviderSettings").mockReturnValue({
				...
				;(provider as any).providerSettingsManager = {
					listConfig: vi.fn().mockResolvedValue([
						{ name: "MaveCode", apiProvider: "mave-gateway" },
						{ name: "Backup MaveCode", apiProvider: "mave-gateway" },
					]),
					getProfile: vi
						.fn()
						.mockResolvedValueOnce({
							apiProvider: "mave-gateway",
							maveSessionToken: "old-token",
							maveGatewayBaseUrl: "https://old.example/api/gateway/v1",
						})
						.mockResolvedValueOnce({
							apiProvider: "mave-gateway",
							maveSessionToken: "old-token",
				...
				await provider.handleMaveCodeCallback("new-token")
			...
			it("logs and posts state when profile persistence fails", async () => {
				...
				expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(
					expect.stringContaining("[handleMaveCodeCallback] Failed to save mave-gateway profile"),
				)
		...
		describe("ensureMaveGatewayProfileSeeded", () => {
			it("does nothing when no cached auth token exists", async () => {
				const { getCachedMaveCodeToken } = await import("../../../services/mave-code-auth")
				vi.mocked(getCachedMaveCodeToken).mockReturnValue("")
				const handleSpy = vi.spyOn(provider, "handleMaveCodeCallback").mockResolvedValue(undefined)

				;(provider as any).providerSettingsManager = {
					listConfig: vi.fn(),
				}

				await (provider as any).ensureMaveGatewayProfileSeeded()

				expect(handleSpy).not.toHaveBeenCalled()
			})

			it("skips seeding when every mave-gateway profile already has the current token and base URL", async () => {
				const { getCachedMaveCodeToken } = await import("../../../services/mave-code-auth")
				vi.mocked(getCachedMaveCodeToken).mockReturnValue("current-token")
				const handleSpy = vi.spyOn(provider, "handleMaveCodeCallback").mockResolvedValue(undefined)
				const postMessageSpy = vi.spyOn(provider, "postMessageToWebview").mockResolvedValue(undefined)
			...
			it("re-seeds when any mave-gateway profile has a stale or missing token", async () => {
				const { getCachedMaveCodeToken } = await import("../../../services/mave-code-auth")
				vi.mocked(getCachedMaveCodeToken).mockReturnValue("fresh-token")
				const handleSpy = vi.spyOn(provider, "handleMaveCodeCallback").mockResolvedValue(undefined)
			...
			it("re-seeds when any mave-gateway profile has a stale base URL", async () => {
				const { getCachedMaveCodeToken } = await import("../../../services/mave-code-auth")
				vi.mocked(getCachedMaveCodeToken).mockReturnValue("current-token")
				const handleSpy = vi.spyOn(provider, "handleMaveCodeCallback").mockResolvedValue(undefined)
