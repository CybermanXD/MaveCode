...
vi.mock("../../../services/mave-code-auth", () => ({
	disconnectMaveCode: vi.fn().mockResolvedValue(undefined),
}))
...
describe("webviewMessageHandler - requestRouterModels", () => {
	...
	it("handles individual provider failures gracefully", async () => {
		...
		mockGetModels
			...
			.mockResolvedValueOnce(mockModels) // mave-gateway
	...
	it("handles Error objects and string errors correctly", async () => {
		...
		mockGetModels
			...
			.mockRejectedValueOnce(new Error("MaveCode error")) // mave-gateway
			.mockRejectedValueOnce(new Error("LiteLLM connection failed")) // litellm
...
describe("maveCodeSignOut", () => {
	beforeEach(() => {
		vi.clearAllMocks()
	})

	it("disconnects MaveCode and clears tokens from all MaveCode profiles", async () => {
		...
		;(mockClineProvider as any).contextProxy = {
			...
			getProviderSettings: vi.fn().mockReturnValue({ apiProvider: "mave-gateway" }),
			getValues: vi.fn().mockReturnValue({ currentApiConfigName: "MaveCode" }),
		}
		;(mockClineProvider as any).providerSettingsManager = {
			...
			getProfile: vi
				...
				.mockResolvedValueOnce({
					...
					maveSessionToken: "token-active",
					maveGatewayModelId: "anthropic/claude-sonnet-4",
				})
				.mockResolvedValueOnce({
					apiProvider: "mave-gateway",
					maveSessionToken: "token-backup",
				}),
			saveConfig,
		}
		;(mockClineProvider as any).upsertProviderProfile = upsertProviderProfile

		await webviewMessageHandler(mockClineProvider, { type: "maveCodeSignOut" })

		expect(disconnectMaveCode).toHaveBeenCalled()
		expect(upsertProviderProfile).toHaveBeenCalledWith(
			"MaveCode",
			expect.not.objectContaining({ maveSessionToken: expect.anything() }),
			true,
		)
