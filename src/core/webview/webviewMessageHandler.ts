...
import { CloudService } from "../../services/cloud/CloudCompatibility"
...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		...
		case "requestRouterModels": {
			...
			const candidates: { key: RouterName; options: GetModelsOptions }[] = [
				...
				{ key: "vercel-ai-gateway", options: { provider: "vercel-ai-gateway" } },
				{
					key: "mave-gateway",
					options: {
						provider: "mave-gateway",
						apiKey: apiConfiguration.maveSessionToken,
						baseUrl: apiConfiguration.maveGatewayBaseUrl,
					},
				},
			]
		...
		case "maveCodeSignOut": {
			try {
				const { disconnectMaveCode } = await import("../../services/mave-code-auth")
				await disconnectMaveCode()

				// Clear maveSessionToken from ALL provider profiles with apiProvider === "mave-gateway".
				// Profiles are user-renameable, so we cannot rely on a hardcoded name like "MaveCode".
				// We must scan all profiles and clear tokens from any that use the mave-gateway provider.
				try {
					...
					for (const entry of allProfiles) {
						if (entry.apiProvider !== providerIdentifiers.maveGateway) {
							continue
						}
						...
						try {
							...
							const { maveSessionToken: _removed, ...cleanedProfile } = profile
							...
							const isThisProfileActive = isMaveGatewayActive && currentApiConfigName === entry.name

							if (isThisProfileActive) {
								await provider.upsertProviderProfile(entry.name, cleanedProfile, true)
								provider.log(
									`[maveCodeSignOut] Cleared maveSessionToken from "${entry.name}" profile and updated in-memory handler`,
								)
							} else if (profile.maveSessionToken) {
								await provider.providerSettingsManager.saveConfig(entry.name, cleanedProfile)
								provider.log(`[maveCodeSignOut] Cleared maveSessionToken from "${entry.name}" profile`)
							}
						} catch (profileError) {
							// Log but continue to the next profile so one failure doesn't
							// leave other profiles holding a stale token.
							provider.log(
								`[maveCodeSignOut] Failed to clear profile token for "${entry.name}": ${profileError instanceof Error ? profileError.message : String(profileError)}`,
							)
						}
					}
				} catch (profileError) {
					...
					provider.log(
						`[maveCodeSignOut] Failed to list profiles for token cleanup: ${profileError instanceof Error ? profileError.message : String(profileError)}`,
					)
				}

				// Token removal changes the top-level UI gate. Re-render the view rather
				// than posting state into the authenticated app that is being discarded.
				await provider.revealSignedOutWebview()
				void vscode.window.showInformationMessage("Signed out of MaveCode.")
			} catch (error) {
				provider.log(
					`Failed to sign out of MaveCode: ${error instanceof Error ? error.message : String(error)}`,
				)
			}
			break
		}
		...
		case "requestMaveCodeRateLimits": {
			try {
				const { MaveCodeAppsScriptClient } = await import("../../services/mavecode-appscript-client")
				const { getMaveCodeBaseUrl, getCachedMaveCodeToken } = await import("../../services/mave-code-auth")
				const sessionToken = getCachedMaveCodeToken()
				if (!sessionToken) throw new Error("MaveCode sign-in is required")
				const values = await new MaveCodeAppsScriptClient(getMaveCodeBaseUrl()).action<
					import("@roo-code/types").OpenAiCodexRateLimitInfo
				>("rate-limits", { sessionToken })
				await provider.postMessageToWebview({ type: "maveCodeRateLimits", values })
			} catch (error) {
				const errorMessage = error instanceof Error ? error.message : "MaveCode usage could not be loaded"
				provider.log(`Error fetching MaveCode rate limits: ${errorMessage}`)
				await provider.postMessageToWebview({ type: "maveCodeRateLimits", error: errorMessage })
			}
			break
		}
