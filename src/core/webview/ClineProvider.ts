...
import { CloudService, getRooCodeApiUrl } from "../../services/cloud/CloudCompatibility"
...
{
	private suppressNextAutomaticMaveCodeSignIn = false
	...
	*/
	...
	async resolveWebviewView(webviewView: vscode.WebviewView | vscode.WebviewPanel) {
		// Re-resolving after authentication must not retain listeners from the gate.
		this.clearWebviewResources()
			...
			webviewView.webview.html = `<!doctype html>
				...
				<head>
					...
					<style nonce="${nonce}">
						...
						button { padding: 9px 14px; border: 1px solid var(--vscode-button-border, transparent); border-radius: 2px; color: var(--vscode-button-foreground); background: var(--vscode-button-background); font: inherit; cursor: pointer; }
				...
				<body>
					<main><h2>Sign in to MaveCode</h2><p>An approved Google account is required before the workspace can be used.</p><p>Complete sign-in in your browser, then return to VS Code.</p><div class="actions"><button id="mavecode-sign-in" type="button">Sign in with Google</button><button id="mavecode-copy-sign-in" class="secondary" type="button">Copy sign-in URL</button></div><p id="mavecode-sign-in-status" role="status" aria-live="polite"></p></main>
					<script nonce="${nonce}">
						const vscode = acquireVsCodeApi();
						const button = document.getElementById("mavecode-sign-in");
						const copyButton = document.getElementById("mavecode-copy-sign-in");
						const status = document.getElementById("mavecode-sign-in-status");
						button.addEventListener("click", () => { button.disabled = true; status.textContent = "Opening your browser…"; vscode.postMessage({ type: "startMaveCodeSignIn" }); });
						copyButton.addEventListener("click", () => { copyButton.disabled = true; status.textContent = "Creating a secure sign-in URL…"; vscode.postMessage({ type: "copyMaveCodeSignInUrl" }); });
						window.addEventListener("message", ({ data }) => { if (data?.type === "maveCodeSignInLaunchComplete") { button.disabled = false; status.textContent = data.launched ? "Continue sign-in in your browser. You can retry if needed." : "Unable to open sign-in. Check the notification, then try again."; } });
						window.addEventListener("message", ({ data }) => { if (data?.type === "maveCodeSignInUrlCopied") { copyButton.disabled = false; status.textContent = data.copied ? "Secure sign-in URL copied. Paste it into your browser." : "Unable to copy the sign-in URL. Try again."; } });
					</script>
				</body></html>`
			const messageDisposable = webviewView.webview.onDidReceiveMessage(async (message: unknown) => {
				if (
					!message ||
					typeof message !== "object" ||
					!["startMaveCodeSignIn", "copyMaveCodeSignInUrl"].includes(
						String((message as { type?: unknown }).type),
					)
				)
					return
				if ((message as { type?: unknown }).type === "copyMaveCodeSignInUrl") {
					let copied = false
					try {
				...
				let launched = false
				try {
					launched = await startMaveCodeSignIn()
				} catch (error) {
					this.log(`[startMaveCodeSignIn] Error: ${error instanceof Error ? error.message : String(error)}`)
				}
				await webviewView.webview.postMessage({ type: "maveCodeSignInLaunchComplete", launched })
			})
			this.webviewDisposables.push(messageDisposable)
			// Render the signed-out home before launching the browser. This keeps the
			// manual button visible if the browser is slow, blocked, or abandoned.
			if (this.suppressNextAutomaticMaveCodeSignIn) {
				this.suppressNextAutomaticMaveCodeSignIn = false
			} else {
				void startMaveCodeSignIn()
					.then((launched) =>
	...
	public async revealSignedOutWebview(): Promise<void> {
		if (!this.view) return
		this.suppressNextAutomaticMaveCodeSignIn = true
		await this.resolveWebviewView(this.view)
	}

	/**
	 * Seeds the mave-gateway provider profile for users who have a cached auth token
	 * but no profile (e.g., users who signed in before MaveCode was added), or
	 * who have an empty/imported profile without a token.
	 * Called once per webview init; handleMaveCodeCallback is idempotent so repeated calls are safe.
	 */
	private async ensureMaveGatewayProfileSeeded(): Promise<void> {
		const { getCachedMaveCodeToken, getMaveCodeBaseUrl } = await import("../../services/mave-code-auth")
		const token = getCachedMaveCodeToken()
		if (!token) return
		const expectedGatewayBaseUrl = getMaveCodeBaseUrl()

		// Check ALL mave-gateway profiles — only skip seeding if every profile has the current token.
		// Using .find() would miss stale tokens in duplicate/renamed profiles since handleMaveCodeCallback
		...
		const maveGatewayProfiles = allProfiles.filter((p) => p.apiProvider === providerIdentifiers.maveGateway)

		if (maveGatewayProfiles.length === 0) {
			this.log("[ensureMaveGatewayProfileSeeded] No mave-gateway profile found, creating one")
		} else {
			let allUpToDate = true

			for (const entry of maveGatewayProfiles) {
				try {
					const fullProfile = await this.providerSettingsManager.getProfile({ name: entry.name })
					if (
						fullProfile.maveSessionToken !== token ||
						fullProfile.maveGatewayBaseUrl !== expectedGatewayBaseUrl
					) {
						allUpToDate = false
						this.log("[ensureMaveGatewayProfileSeeded] Existing mave-gateway profile is stale, updating")
						break
					}
				} catch {
					allUpToDate = false
					this.log("[ensureMaveGatewayProfileSeeded] Failed to read existing profile, will re-seed")
					break
				}
			}
			...
			if (allUpToDate) {
				...
				if (currentSettings.apiProvider !== providerIdentifiers.maveGateway) {
					await this.activateProviderProfile({ name: maveGatewayProfiles[0].name })
				}
				const { postMaveGatewayCredentialsReady } = await import("../../services/mave-gateway-credentials-sync")
				postMaveGatewayCredentialsReady((message) => this.postMessageToWebview(message))
				return
			}
		}

		// User has token but either no profile, some profiles without token, or stale tokens — seed all
		await this.handleMaveCodeCallback(token)
	}
	...
	async handleMaveCodeCallback(token: string) {
		...
		// Save the mave-gateway provider profile with the session token so that
		// MaveGatewayHandler can authenticate without any manual user input.
		//
		// activate: true ONLY if MaveCode is already the active profile — this pushes
		...
		// must explicitly select MaveCode in settings if they want to use it.
		try {
			const { apiConfiguration } = await this.getState()
			const currentSettings = this.contextProxy.getProviderSettings()
			const currentApiConfigName = this.contextProxy.getValues().currentApiConfigName

			// Derive the gateway base URL from MAVE_CODE_BASE_URL so that non-prod environments
			...
			const { getMaveCodeBaseUrl } = await import("../../services/mave-code-auth")
			const derivedGatewayBaseUrl = getMaveCodeBaseUrl()

			// Check if MaveCode is the currently active profile by apiProvider identity,
			// not by profile name (profile names are user-renameable).
			const isMaveGatewayActive = currentSettings.apiProvider === providerIdentifiers.maveGateway

			// Always scan ALL profiles and update every mave-gateway profile with the new token.
			...
			if (zooProfiles.length === 0) {
				...
				const newConfiguration: ProviderSettings = {
					...
					maveSessionToken: token,
					maveGatewayModelId: apiConfiguration.maveGatewayModelId,
					maveGatewayBaseUrl: derivedGatewayBaseUrl,
				}
				// Activate only if mave-gateway was the active provider (shouldn't happen if
				// no profiles exist, but defensive).
				await this.upsertProviderProfile("MaveCode", newConfiguration, true)
			} else {
			...
			this.log(
				`[handleMaveCodeCallback] Failed to save mave-gateway profile: ${
					error instanceof Error ? error.message : String(error)
				}`,
			)
		}
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		try {
			...
			maveCodeState = {
				...
				maveCodeBaseUrl: getMaveCodeBaseUrl(),
				deviceName: os.hostname(),
			}
		} catch {
			// Keep the default unauthenticated state if the optional MaveCode auth service is unavailable.
		}
	...
	private logWebviewHiddenDiagnostics(): void {
		...
		this.log(
			`[MaveCode] Webview hidden during active task.\n` +
				`  taskId:       ${task.taskId}\n` +
