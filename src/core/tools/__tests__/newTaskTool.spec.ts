...
describe("newTaskTool", () => {
	...
	describe("VSCode setting: newTaskRequireTodos", () => {
		...
		it("should check VSCode setting with Package.name configuration key", async () => {
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("mave-code")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})

		it("should use current Package.name value (mave-code-nightly) when accessing VSCode configuration", async () => {
			...
			;(pkg.Package as any).name = "mave-code-nightly"
			...
			try {
				...
				expect(mockGetConfiguration).toHaveBeenCalledWith("mave-code-nightly")
				expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
			} finally {
