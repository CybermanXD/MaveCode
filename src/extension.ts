...
import { initMaveCodeAuth } from "./services/mave-code-auth"
...
/**
 * Check if we should auto-open the MaveCode sidebar after switching to a worktree.
...
async function checkWorktreeAutoOpen(
	...
	try {
		...
		if (normalizePath(currentPath) === normalizePath(worktreeAutoOpenPath)) {
			...
			outputChannel.appendLine(`[Worktree] Auto-opening MaveCode sidebar for worktree: ${worktreeAutoOpenPath}`)

			// Open the MaveCode sidebar with a slight delay to ensure UI is ready
			setTimeout(async () => {
				try {
					await vscode.commands.executeCommand("mave-code.plusButtonClicked")
				} catch (error) {
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// Initialize MaveCode auth service for extension session token management.
	await initMaveCodeAuth(context)
	...
	// Initialize the local provider. MaveCode does not initialize or depend on
	// the inherited cloud authentication/settings runtime.
	...
	const socketPath = process.env.MAVE_CODE_IPC_SOCKET_PATH
