...
describe("LiteLLMHandler", () => {
	...
	describe("session ID header", () => {
		...
		it("should send the X-Mave-Session-ID header when a taskId is provided", async () => {
			mockCreate.mockReturnValue({
				withResponse: vi.fn().mockResolvedValue({ data: makeMockStream() }),
			})
			...
			expect(requestHeaders).toMatchObject({ "X-Mave-Session-ID": "task-123" })
		})

		it("should not send the X-Mave-Session-ID header when no taskId is provided", async () => {
			mockCreate.mockReturnValue({
				withResponse: vi.fn().mockResolvedValue({ data: makeMockStream() }),
			})
			...
			const requestHeaders = mockCreate.mock.calls[0][1]?.headers
			expect(requestHeaders).not.toHaveProperty("X-Mave-Session-ID")
		})

		it("should not send the X-Mave-Session-ID header when taskId is an empty string", async () => {
			mockCreate.mockReturnValue({
				withResponse: vi.fn().mockResolvedValue({ data: makeMockStream() }),
			})
