...
import { getMaveGatewayModels } from "../mave-gateway"
...
const mockGetMaveGatewayModels = getMaveGatewayModels as Mock<typeof getMaveGatewayModels>
...
describe("getModelsFromCache disk fallback", () => {
	...
	it("isolates authenticated users through the canonical MaveCode identifier", () => {
		...
		const result = getModelsFromCache(providerIdentifiers.maveGateway)
...
describe("empty cache protection", () => {
	...
	describe("getModels", () => {
		...
		it("re-arms the empty-response throttle after a non-empty response from an auth-scoped provider", async () => {
			// mave-gateway is auth-scoped and skips caching entirely, but a non-empty response
			// must still clear the throttle so a later empty response is reported again.
			mockGetMaveGatewayModels.mockResolvedValueOnce({})

			await getModels({ provider: providerIdentifiers.maveGateway, apiKey: "test-key" })
			...
			const mockModels = {
				"mave-gateway/model": {
					...
					contextWindow: 128000,
					supportsPromptCache: false,
					description: "MaveCode model",
				},
			}
			mockGetMaveGatewayModels.mockResolvedValueOnce(mockModels)
...
describe("MODEL_CACHE_EMPTY_RESPONSE throttling", () => {
	...
	let freshMockGetMaveGatewayModels: Mock<typeof getMaveGatewayModels>
	...
	beforeEach(async () => {
		...
		const maveGatewayModule = await import("../mave-gateway")
		...
		freshMockGetMaveGatewayModels = maveGatewayModule.getMaveGatewayModels as Mock<typeof getMaveGatewayModels>
	...
	it("throttles mave-gateway independently per session token, even though caching itself is skipped", async () => {
		// mave-gateway is auth-scoped (see AUTH_SCOPED_PROVIDERS) and never persists to the
		...
		freshMockGetMaveGatewayModels.mockResolvedValue({})

		await freshGetModels({ provider: providerIdentifiers.maveGateway, apiKey: "account-a-token" })
		await freshGetModels({ provider: providerIdentifiers.maveGateway, apiKey: "account-a-token" })
		expect(FreshTelemetryService.instance.captureEvent).toHaveBeenCalledTimes(1)

		await freshGetModels({ provider: providerIdentifiers.maveGateway, apiKey: "account-b-token" })
		expect(FreshTelemetryService.instance.captureEvent).toHaveBeenCalledTimes(2)
	})

	it("throttles mave-gateway independently per gateway baseUrl", async () => {
		...
		await freshGetModels({
			provider: providerIdentifiers.maveGateway,
			apiKey: "token",
	...
	it("never shares results across different mave-gateway credentials (auth isolation)", async () => {
		// Auth-scoped providers (see AUTH_SCOPED_PROVIDERS) bypass dedupedFetch entirely --
		// shouldSkipCache is true for mave-gateway, so every call fires its own provider fetch
		...
		const promiseA = freshGetModels({ provider: providerIdentifiers.maveGateway, apiKey: "account-a-token" })
		const promiseB = freshGetModels({ provider: providerIdentifiers.maveGateway, apiKey: "account-b-token" })

		expect(freshMockGetMaveGatewayModels).toHaveBeenCalledTimes(2)
	...
	it("never deduplicates concurrent mave-gateway fetches, even for the same token", async () => {
		...
		await Promise.all([
			freshGetModels({ provider: providerIdentifiers.maveGateway, apiKey: "same-token" }),
			freshGetModels({ provider: providerIdentifiers.maveGateway, apiKey: "same-token" }),
		])
