...
// stale results from a previously-cached server. mave-gateway is included too: although it's
...
const URL_SCOPED_PROVIDERS: ReadonlySet<RouterName> = new Set([
	...
	providerIdentifiers.maveGateway,
])
...
// mave-gateway and kimi-code are included so a sign-out/sign-in cycle to a different account
...
const KEY_SCOPED_PROVIDERS: ReadonlySet<RouterName> = new Set([
	...
	providerIdentifiers.maveGateway, // Per-session-token account identity
...
async function fetchModelsFromProvider(options: GetModelsOptions): Promise<ModelRecord> {
	...
	switch (provider) {
		...
		case providerIdentifiers.moonshot:
			...
			break
		case providerIdentifiers.maveGateway:
			models = await getMaveGatewayModels({ maveSessionToken: options.apiKey, maveGatewayBaseUrl: options.baseUrl })
			break
...
export const refreshModels = async (options: GetModelsOptions): Promise<ModelRecord> => {
	...
	try {
		...
		// For auth-scoped providers (mave-gateway) we MUST NOT return cached models from a prior
...
export function getModelsFromCache(options: GetModelsOptions | ProviderName): ModelRecord | undefined {
	// Auth-scoped providers (e.g. mave-gateway) must never be served from cache --
