...
describe("convertToVsCodeLmMessages", () => {
	...
	it("should log MaveCode branded warning when asObjectSafe fails to parse invalid JSON string", () => {
		const consoleWarnSpy = vi.spyOn(console, "warn").mockImplementation(() => {})
		...
		expect(consoleWarnSpy).toHaveBeenCalledWith(
			"MaveCode <Language Model API>: Failed to parse object:",
			expect.any(Error),
		)
...
describe("extractTextCountFromMessage", () => {
	...
	it("should log MaveCode branded warning when tool call input stringify fails", () => {
		...
		expect(consoleErrorSpy).toHaveBeenCalledWith(
			"MaveCode <Language Model API>: Failed to stringify tool call input:",
			expect.any(Error),
		)
