...
function asObjectSafe(value: unknown): object {
	...
	try {
		...
		console.warn("MaveCode <Language Model API>: Failed to parse object:", error)
		return {}
	}
}
			...
			if (item instanceof vscode.LanguageModelToolCallPart) {
				...
				if (item.input && Object.keys(item.input).length > 0) {
					try {
						...
						console.error("MaveCode <Language Model API>: Failed to stringify tool call input:", error)
					}
				}
			}
		}
