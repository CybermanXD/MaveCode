...
const defaultMaveCodeAuthPageUrl = "https://mavecode.blogspot.com/"
// Deployment-specific endpoints must be supplied by the release pipeline and must
// never be committed into the public source repository.
const defaultMaveCodeBackendUrl = ""
...
async function main() {
	...
	const buildOptions = {
		...
		define: {
			...
			"process.env.MAVE_CODE_AUTH_PAGE_URL": JSON.stringify(
				process.env.MAVE_CODE_AUTH_PAGE_URL || defaultMaveCodeAuthPageUrl,
			),
			"process.env.MAVE_CODE_BACKEND_URL": JSON.stringify(
				process.env.MAVE_CODE_BACKEND_URL || defaultMaveCodeBackendUrl,
			),
		},
	}
