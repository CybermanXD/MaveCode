...
export const DIFF_VIEW_LABEL_CHANGES = "Original ↔ MaveCode's Changes"
...
export class DiffViewProvider {
	...
	async saveChanges(
		...
		await this.keepOrCloseEditedFile(
			...
			saveState?.autoCloseMaveCodeOpenedFiles ?? DEFAULT_AUTO_CLOSE_MAVECODE_OPENED_FILES,
			saveState?.autoCloseMaveCodeOpenedFilesAfterUserEdited ?? DEFAULT_AUTO_CLOSE_MAVECODE_OPENED_FILES_AFTER_USER_EDITED,
			saveState?.autoCloseMaveCodeOpenedNewFiles ?? DEFAULT_AUTO_CLOSE_MAVECODE_OPENED_NEW_FILES,
		)
	...
	async revertChanges(): Promise<void> {
		...
		if (!fileExists) {
			...
			await this.keepOrCloseEditedFile(
				absolutePath,
				false,
				revertState?.autoCloseMaveCodeOpenedFiles ?? DEFAULT_AUTO_CLOSE_MAVECODE_OPENED_FILES,
				revertState?.autoCloseMaveCodeOpenedFilesAfterUserEdited ??
					DEFAULT_AUTO_CLOSE_MAVECODE_OPENED_FILES_AFTER_USER_EDITED,
				revertState?.autoCloseMaveCodeOpenedNewFiles ?? DEFAULT_AUTO_CLOSE_MAVECODE_OPENED_NEW_FILES,
			)
		}
	...
	//   2. editType==="create" AND autoCloseMaveCodeOpenedNewFiles -> close the new file's tab.
	//   3. userTouchedDocument OR keepIfTouchedDiff -> the "keep if touched" guard
	//      applies; it is overridden (close) only when BOTH autoCloseMaveCodeOpenedFiles
	//      and autoCloseMaveCodeOpenedFilesAfterUserEdited are enabled. The override is a
	//      refinement of the base auto-close, so it has no effect when the base
	//      setting is off.
	//   4. autoCloseMaveCodeOpenedFiles=false -> keep the transiently-opened tab.
	//   5. autoCloseMaveCodeOpenedFiles=true -> close the transiently-opened tab.
	//
	// The default value of autoCloseMaveCodeOpenedFiles is false (opt-in), so by default
	...
	private async keepOrCloseEditedFile(
		...
		// New files on the accept path: close when autoCloseMaveCodeOpenedNewFiles is enabled.
		if (this.editType === "create" && autoCloseMaveCodeOpenedNewFiles) {
			await this.closeFileTab(absolutePath)
			return
		}
		...
		if (userInteracted) {
			...
			if (autoCloseMaveCodeOpenedFiles && autoCloseMaveCodeOpenedFilesAfterUserEdited) {
				await this.closeFileTab(absolutePath)
			} else {
				await this.showEditedFileWithoutDisruptingFocus(absolutePath)
			}
		...
		// Transient tab opened by MaveCode: close only when auto-close is enabled (opt-in);
		// keep and re-show it otherwise (the default).
		if (autoCloseMaveCodeOpenedFiles) {
			await this.closeFileTab(absolutePath)
		} else {
			await this.showEditedFileWithoutDisruptingFocus(absolutePath)
		}
