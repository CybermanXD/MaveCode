...
export class Terminal extends BaseTerminal {
	...
	constructor(id: number, terminal: vscode.Terminal | undefined, cwd: string) {
		...
		if (terminal) {
			...
			const options: vscode.TerminalOptions = { cwd, name: "MaveCode", iconPath, env }
			...
			if (profileShell?.shellPath) {
				...
				// MaveCode's shell-integration controls authoritative.
				if (profileShell.env) {
	 ...
	 * `.vscode/settings.json` to select an executable for MaveCode to launch.
	 */
