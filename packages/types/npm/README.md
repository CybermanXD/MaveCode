...
The MaveCode extension exposes an API that can be used by other extensions.
...
const extension = vscode.extensions.getExtension<RooCodeAPI>("MaveCode.mave-code")
...
await api.startNewTask("Hello, MaveCode API! Let's make a new project...")
...
**NOTE:** To ensure that the `MaveCode.mave-code` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:

```json
"extensionDependencies": ["MaveCode.mave-code"]
```
