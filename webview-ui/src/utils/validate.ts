...
export function validateApiConfiguration(
	...
	const keysAndIdsPresentErrorMessage = validateModelsAndKeysProvided(apiConfiguration, maveCodeIsAuthenticated)
...
function validateModelsAndKeysProvided(
	...
	switch (apiConfiguration.apiProvider) {
		...
		case providerIdentifiers.maveGateway:
			if (!apiConfiguration.maveSessionToken && !maveCodeIsAuthenticated) {
				return i18next.t("settings:validation.maveGatewaySignIn")
			}
			break
 ...
 *
 * MaveCode's sign-in error is rendered inline by the `MaveGateway` provider
 * component, so we skip the keys/sign-in check here. Organization provider
 * restrictions still need to be enforced for mave-gateway, so the org allowlist
...
export function validateApiConfigurationExcludingModelErrors(
	...
	if (apiConfiguration.apiProvider !== providerIdentifiers.maveGateway) {
