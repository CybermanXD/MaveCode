	...
	describe("MaveCode validation", () => {
		describe("validateApiConfiguration (welcome-view entry point)", () => {
			it("returns a sign-in error when neither profile token nor Zoo auth is present", () => {
				...
				expect(result).toBe("settings:validation.maveGatewaySignIn")
			})

			it("returns undefined when MaveCode auth is active without a profile token", () => {
				const config: ProviderSettings = {
					apiProvider: "mave-gateway",
					maveGatewayModelId: "anthropic/claude-sonnet-4",
				}
		...
		describe("validateApiConfigurationExcludingModelErrors (settings form)", () => {
			// The settings form short-circuits mave-gateway and renders the sign-in
			// error inline in `MaveGateway.tsx`, so this entry point must never
			// surface a mave-gateway-specific error regardless of auth state.
			it("returns undefined for mave-gateway when unauthenticated and no token", () => {
				const config: ProviderSettings = {
					apiProvider: "mave-gateway",
					maveGatewayModelId: "anthropic/claude-sonnet-4",
				}
			...
			it("returns undefined for mave-gateway when a profile token is set", () => {
				const config: ProviderSettings = {
					apiProvider: "mave-gateway",
					maveGatewayModelId: "anthropic/claude-sonnet-4",
					maveSessionToken: "mave_ext_test_token",
			...
			it("surfaces PROVIDER_NOT_ALLOWED for mave-gateway when organization disallows it", () => {
				const orgWithoutMaveGateway: OrganizationAllowList = {
					allowAll: false,
					providers: {
