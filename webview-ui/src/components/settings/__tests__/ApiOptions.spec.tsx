...
import { openAiCodexDefaultModelId, maveGatewayDefaultModelId } from "@roo-code/types"
...
describe("ApiOptions", () => {
	...
	it("initializes maveGatewayModelId to its default when switching provider to mave-gateway", () => {
		// Regression: mave-gateway was previously missing from PROVIDER_MODEL_CONFIG, so switching
		// providers never seeded maveGatewayModelId. Configs were left without a model id, which
		...
		const providerSelectContainer = screen.getByTestId("provider-select")
		const providerSelect = providerSelectContainer.querySelector("select") as HTMLSelectElement
		fireEvent.change(providerSelect, { target: { value: "mave-gateway" } })

		expect(mockSetApiConfigurationField).toHaveBeenCalledWith("apiProvider", "mave-gateway")
		expect(mockSetApiConfigurationField).toHaveBeenCalledWith("maveGatewayModelId", maveGatewayDefaultModelId, false)
	})
