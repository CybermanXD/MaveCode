...
export const OpenAICodexRateLimitDashboard: React.FC<OpenAICodexRateLimitDashboardProps> = ({
	...
	const fetchRateLimits = useCallback(() => {
		...
		setIsLoading(true)
		setError(null)
		vscode.postMessage({ type: provider === "maveCode" ? "requestMaveCodeRateLimits" : "requestOpenAiCodexRateLimits" })
	}, [isAuthenticated, provider])

	useEffect(() => {
		const handleMessage = (event: MessageEvent) => {
			const message = event.data
			const expectedType = provider === "maveCode" ? "maveCodeRateLimits" : "openAiCodexRateLimits"
