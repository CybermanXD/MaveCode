// npx vitest src/components/ui/hooks/__tests__/useMaveGatewayRouterModelsSync.spec.ts
...
import { useMaveGatewayRouterModelsSync } from "../useMaveGatewayRouterModelsSync"
...
const setAuthenticated = (maveCodeIsAuthenticated: boolean) => {
	mockUseExtensionState.mockReturnValue({ maveCodeIsAuthenticated })
}

const renderSyncHook = (queryClient: QueryClient) =>
	renderHook(() => useMaveGatewayRouterModelsSync(), {
...
describe("useMaveGatewayRouterModelsSync", () => {
	it("does not fetch when the user is not authenticated", async () => {
		...
		window.dispatchEvent(new MessageEvent("message", { data: { type: "maveGatewayCredentialsReady" } }))

		await Promise.resolve()
		expect(mockFetchRouterModels).not.toHaveBeenCalled()
	})

	it("fetches mave-gateway models on the maveGatewayCredentialsReady message when authenticated", async () => {
		...
		await waitFor(() => expect(mockFetchRouterModels).toHaveBeenCalledWith("mave-gateway"))
	})
	...
	it("does not overwrite the cache when the fetch returns no mave-gateway models", async () => {
		setAuthenticated(true)
		mockFetchRouterModels.mockResolvedValue(asRouterModels({ "mave-gateway": {} }))
		const queryClient = makeQueryClient()
		...
		const cached = queryClient.getQueryData<RouterModels>(["routerModels", "all"])
		expect(cached?.["mave-gateway"]).toBeUndefined()
		// The whole cache must survive an empty result, not just the mave-gateway key.
