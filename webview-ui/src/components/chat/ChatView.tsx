...
export interface ChatViewProps {
	isHidden: boolean
	/** Deprecated compatibility fields; release announcements are intentionally disabled. */
...
const ChatViewComponent: React.ForwardRefRenderFunction<ChatViewRef, ChatViewProps> = ({ isHidden }, ref) => {
	...
	useEffect(() => {
		...
		// Chat controls are local React state. Without resetting them, a task
		// switch can briefly inherit the previous task's in-flight API state.
		// A message submitted during that window is incorrectly queued on the
		// restored task and is then consumed by its resume ask.
		setSendingDisabled(false)
		setClineAsk(undefined)
		setEnableButtons(false)
		setPrimaryButtonText(undefined)
	...
	const isStreaming = useMemo(() => {
		...
		const isLastAsk = !!lastMessage?.ask
		...
		// A complete ask always means the extension is waiting for the user, not
		// for an API response. This is especially important for restored tasks:
		// legacy history can contain an api_req_started record without a cost.
		// Treating that old record as active makes the restored chat look stuck
		// and causes the user's next message to be queued instead of answering
		// resume_task/resume_completed_task.
		if (isLastAsk && lastMessage?.partial !== true) {
			return false
		}
		...
		if (isLastMessagePartial) {
			...
			if (
				...
				try {
					const requestInfo = JSON.parse(lastApiReqStarted.text) as {
						cost?: unknown
						cancelReason?: unknown
					}
					cost = requestInfo.cost
					cancelReason = requestInfo.cancelReason
				} catch {
					// A malformed historical request marker is not evidence that a
