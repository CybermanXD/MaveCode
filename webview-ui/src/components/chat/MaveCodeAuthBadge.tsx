...
interface MaveCodeAuthBadgeProps {
	className?: string
}
...
export const MaveCodeAuthBadge: React.FC<MaveCodeAuthBadgeProps> = ({ className }) => {
	const {
		maveCodeIsAuthenticated,
		maveCodeUserName,
		maveCodeUserEmail,
		maveCodeUserImage,
	} = useExtensionState()
	...
	const showImage = maveCodeIsAuthenticated && maveCodeUserImage && !imageError
	const avatarColor = getAvatarColor(maveCodeUserEmail || maveCodeUserName || "ZC")
	const avatarButtonStyle: CSSProperties | undefined =
		maveCodeIsAuthenticated && !showImage ? { backgroundColor: avatarColor } : undefined
	...
	const handleSignOut = () => {
		vscode.postMessage({ type: "maveCodeSignOut" })
		setIsOpen(false)
	}
	...
	return (
		<div ref={ref} className={cn("relative ml-2", className)}>
			...
			<button
				...
				title={maveCodeIsAuthenticated ? `MaveCode: ${maveCodeUserEmail || "Connected"}` : "Sign in to MaveCode"}>
				{maveCodeIsAuthenticated ? (
					showImage ? (
						<img
							src={maveCodeUserImage}
							alt="avatar"
							className="size-full rounded-full object-cover"
							onError={() => setImageError(true)}
						/>
					) : (
						<span>{getInitials(maveCodeUserName, maveCodeUserEmail)}</span>
					)
				) : (
					// Person icon SVG
					<svg
			...
			{isOpen && (
				<div
					...
					{!maveCodeIsAuthenticated ? (
						<div className="px-3.5 py-2.5 text-[13px] text-vscode-descriptionForeground">
							Use the MaveCode sign-in home to authenticate.
						</div>
					) : (
						<>
							{maveCodeUserEmail && (
