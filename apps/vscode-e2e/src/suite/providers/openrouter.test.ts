...
suite("OpenRouter provider", function () {
	...
	test("Should identify as MaveCode in outbound DEFAULT_HEADERS", async () => {
		requests.length = 0
		...
		for (const captured of requests) {
			assert.strictEqual(captured.xTitle, "MaveCode", "X-Title header should identify the extension as MaveCode")
			assert.strictEqual(
				captured.httpReferer,
				"https://github.com/MaveCode-Org/MaveCode",
				"HTTP-Referer header should point to the MaveCode repository",
			)
			assert.ok(
				captured.userAgent?.startsWith("MaveCode/"),
				`User-Agent should start with "MaveCode/" — got: ${captured.userAgent}`,
			)
		}
	})
})
