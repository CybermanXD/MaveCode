...
suite("Bedrock provider", function () {
	...
	test("Should complete a task end-to-end via AWS Bedrock with MaveCode# user-agent", async () => {
		...
		if (mockServer) {
			// Verify the AWS SDK transmitted the MaveCode# userAgentAppId.
			...
			assert.ok(userAgent.includes("MaveCode#"), `user-agent should contain "MaveCode#" — got: ${userAgent}`)
		} else {
			...
			assert.ok(true, "Task completed successfully via Bedrock with MaveCode# userAgentAppId")
		}
	})
